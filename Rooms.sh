...
function Room(roomname, introtext, roompic){
	...
	this.commands = ["cd", "ls", "less", "man", "help", "exit", "pwd", "mv"];
...
Room.prototype.mv = function(args){
	if (args.length != 2){
		return "You need to move the bookshelf out of the way to reveal the hidden room. Use mov [bookshelf] [placeB].";
	} else {
		...
		if ((item_name_to_move >= 0) && (this.childrenStringArray().indexOf(args[1]) >= 0)){
			...
			if ((args[0] === "Bookshelf") && (this.room_name === "Hidden_Room")){
				this.ev.fire("mvBookshelf");
			}
		...
		for (var i = 0; i < args.length; i++){
			if (this.getItemFromName(args[i]) != -1){
				if (this.getItemFromName(args[i]).valid_cmds.indexOf("rm") > 0){
					...
					if (removedItem.itemname === "Bookshelf" && this.room_name === "Hidden_Room"){
						this.ev.fire("rmBookshelf")
					}
...
Room.prototype.grep = function(args){
	...
	return "You haven't learned this command tool yet.";
};
...
Room.prototype.touch = function(args){
	...
	return "You haven't learned this tool yet.";
};
